...
<body class="gray-bg">
				...
				<div class="ibox float-e-margins">
							...
							<div class="col-sm-12">
								<div class="input-group">
									<form
										...
										<input type="text" placeholder="昵称查询" name="name"
											...
											<div class="input-group m-bot15"
												...
												<input type="text" name="timeEnd" value="${timeEnd}"
													class="form-control" placeholder="请输入直播截止时间"
													onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"
													readonly>
											</div>
											<select
										...
										<button type="submit"
											class="btn btn-sm btn-primary  pull-left" style="height: 35px;">搜索</button>
									</form>
								</div>
							</div>
						</div>
							...
							<table class="table table-striped table-bordered">
								<thead>
									<tr>
										...
										<th style="text-align: center;">观看峰值</th>
										<th style="text-align: center;">添加时间</th>
										<th style="text-align: center;">回放状态</th>
					...
					<c:forEach items="${pagination.list}" var="list">
						<tr>
							...
							<td align="center">${list.watchnumber}</td>
							<td align="center">${list.addtime}</td>
							<td align="center"><c:if test="${list.playbackstatus==0}">
							...
							<td align="center" >
								...
								<div class="btn-group btn-group-xs">
									...
									<a href="${pagecontext.request.contextPath}/room/look.do?id=${list.id}" class="btn btn-success">查看详情</a>
    ...
    <script type="text/javascript">
		...
		function del(iid) {
			...
			$.get("${pageContext.request.contextPath}/room/user_deluser.do", {
				id : iid
			}, function(res) {
				if ("ok" == res) {
					alert("删除成功！！");
		...
		function liveEnabled(iid) {
			...
			$.get( "${pageContext.request.contextPath}/room/isplaybackstatus.do",
				 { id : iid },
				function(res) {
					if ("ok" == res) {
						alert("启用成功！！");
		...
		function liveDisable(iid) {
			...
			$.get("${pageContext.request.contextPath}/room/noplaybackstatus.do",
				{id : iid},
				function(res) {
					if ("ok" == res) {
						alert("禁用成功！！");
